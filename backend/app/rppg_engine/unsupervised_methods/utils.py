"""Utility functions for rPPG signal processing."""
...
def detrend(input_signal, lambda_value):
    """
    Detrend signal using sparse matrix regularization.
    
    Args:
        input_signal: Input signal to detrend
        lambda_value: Smoothness parameter (typically 100)
...
def process_video(frames):
    """
    Calculate spatial average of RGB values for each frame.
    
    Args:
        frames: Array of video frames [num_frames, height, width, channels]
        
    Returns:
        RGB array of shape [1, 3, num_frames]
    """
    RGB = []
    for frame in frames:
